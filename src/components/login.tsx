    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <div className="flex flex-col align-middle items-start justify-start w-full">
          <input style={{cursor:"none"}} type="password" placeholder="password" id="password" onChange={(e) => setPassword(e.target.value)} className="mb-2 p-2 text-white w-full bg-black border-[3px] border-white rounded-md font-press-start focus:shadow-md focus:shadow-white focus:outline-none transition-all duration-400 ease-in-out" /> 
        </div>
